...
def authenticate(email, password):
    ...
    if r_auth.status_code == 200:
        ...
        if 'access_token' not in data:
            logger.critical('Authentication succeeded but response is missing access_token.')
            return None, None
...
def get_alerts(session, headers):
    ...
    for counter, alert in enumerate(r_alerts.json()):
        ...
        pagination_data = r_pagination.json()
        if 'pagination' not in pagination_data:
            logger.warning(f'Unexpected response for alert {alert["id"]}, skipping pagination data.')
            continue
        pagination = pagination_data['pagination']
        data_dict['nb_pages'].append(pagination['nbPages'])
        data_dict['all'].append(pagination['totals']['all'])
        data_dict['read'].append(pagination['totals']['read'])
        data_dict['unread'].append(pagination['totals']['unread'])
        data_dict['favorite'].append(pagination['totals']['favorite'])
        data_dict['contact'].append(pagination['totals']['contact'])
        data_dict['deleted'].append(pagination['totals']['deleted'])
...
def get_appart_response(session, row_tuple):
    ...
    try:
        ...
        logger.warn(f'Connection interrupted: {e}. Waiting 30 seconds before retrying.')
        time.sleep(30)
